  ...
  $.fn.drawDoughnutChart = function (data, options) {
    ...
    for (var i = 0, len = data.length; i < len; i++) {
      ...
      $paths[i] = $(document.createElementNS('http://www.w3.org/2000/svg', 'path')).attr({
        ...
        "fill": data[i].color,
        "data-order": i
      }).appendTo($pathGroup).on("mouseenter", pathMouseEnter).on("mouseleave", pathMouseLeave).on("mousemove", pathMouseMove).on("click", pathClick);
    } //Animation start
    ...
    function pathMouseEnter(e) {
      ...
      if (settings.showTip) {
        $tip.text(data[order].title + ": " + data[order].value + " " + settings.tipUnit).fadeIn(200);
      }
    ...
    function shortKInt(_int) {
      _int = _int.toString();
      var strlen = _int.length;
      if (strlen < 5) return _int;
      if (strlen < 8) return '<span title="' + _int + '">' + _int.substring(0, strlen - 3) + 'K</span>';
      return '<span title="' + _int + '">' + _int.substring(0, strlen - 6) + 'M</span>';
    }

    function getScaleFontSize(block, newText) {
      block.css('font-size', '');
      newText = newText.toString().replace(/(<([^>]+)>)/ig, "");
      var newFontSize = block.width() / newText.length * settings.ratioFont; // Not very good : http://stephensite.net/WordPressSS/2008/02/19/how-to-calculate-the-character-width-accross-fonts-and-points/
      ...
      if (maxCharForDefaultFont < 0) return newFontSize + 'px';else return '';
    }
    /**
    function getScaleFontSize(block, newText) {
    	block.css('font-size', '');
